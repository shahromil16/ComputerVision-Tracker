function precisions = show_precision(positions, ground_truth, title)
%SHOW_PRECISION
...
%   http://www.isr.uc.pt/~henriques/
	...
	if size(positions,1) ~= size(ground_truth,1),
        ...
        val = min(size(positions,1),size(ground_truth,1));
        positions = positions(1:val,:);
        ground_truth = ground_truth(1:val,:);
